...
3. **Known DOM facts (Aug 2026)** — keep this list current when you learn something new:
   ...
   - dead-page banners live in `[role="alert"]`, `#side header`, aria-modal dialogs, `[data-icon="alert-phone"|"alert-computer"]`; the needles are `WASession.DEAD_TEXTS`
...
7a. **A job's LIST CONTENT belongs to whoever sent `/start`** (`lock_owner`); until `/run`, `/cancel` or `job_ttl_seconds` of silence, other senders' plain messages are ignored (logged with the reason). **Commands (`/…`, `1`, `2`) are always open to everyone, and a message that tags the bot (`@…`) always counts** — the group must never be locked out of its own bot. `/ping` is the liveness check anyone can run. Group-name matching is on letters+digits only (emoji vanish from `innerText`) and symmetric.
...
7c. **A page that polls fine but never changes is dead, not quiet.** Headless WhatsApp Web drops its phone link ("Computer not connected", "Trying to reach phone", a Refresh/Log-in screen, "open on another computer") and keeps rendering the same messages for ever. Every heartbeat (30 polls) the bot runs `WASession.dead_reason()` and reloads if it names a state; when idle with nothing new for `idle_reload_minutes` it reloads anyway. The heartbeat prints rows vs parsed rows and the last message, and once per run a sample of a row the reader cannot parse — the log must be enough to fix a DOM change without a live probe.
